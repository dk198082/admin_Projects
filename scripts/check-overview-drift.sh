...
KNOWN_ROUTES="health.ts entra.ts index.ts auth.ts accessCheck.ts accessCheck.test.ts users.ts roles.ts appsResources.ts appsResources.test.ts grants.ts security.ts audit.ts audit.test.ts sync.ts sync.test.ts apiKeys.ts apiKeys.test.ts apiKeys.auth-enforcement.test.ts permissionMatrix.ts accessMapping.ts workOrderPurge.ts requireAuth.integration.test.ts"
